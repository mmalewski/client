...
private slots:
    ...
    #ifdef _WIN32
        QCOMPARE(exclude_check_file("file_trailing_space "), CSYNC_FILE_EXCLUDE_TRAILING_SPACE);

        QCOMPARE(exclude_check_file("file_trailing_dot."), CSYNC_FILE_EXCLUDE_INVALID_CHAR);
        QCOMPARE(exclude_check_file("AUX"), CSYNC_FILE_EXCLUDE_INVALID_CHAR);
        QCOMPARE(exclude_check_file("file_invalid_char<"), CSYNC_FILE_EXCLUDE_INVALID_CHAR);
    #endif
    ...
    void check_csync_excluded_performance()
    {
        ...
        // Being able to use QElapsedTimer for measurement would be nice...
        {
            ...
            QCOMPARE(totalRc, CSYNC_NOT_EXCLUDED); // mainly to avoid optimization
